...
if [ ! -e $log_file ]
then
	/bin/echo "TC PASS? ,Yes , There is no cmd which uses the relative path." > $result_file
else
	/bin/echo "TC PASS? ,No , There are cmds which uses the relatvie path." > $result_file
fi
