...
RESULT_FAILED_SERVICES="/usr/share/security-config/output/root_test/result_failed_list"
RESULT_NEW_ROOT_SERVICES="/usr/share/security-config/output/root_test/result_new_root_list"
...
checkNonRootList(){
	...
	do
		...
		if [ $1 = $service_name ]; then
			...
			if [ $service_uid = $process_uid ]; then
				...
				/bin/echo "$1($2), expected uid is $service_uid, but real uid is $process_uid, it is not appropriate uid" >> $RESULT_FAILED_SERVICES 
				/bin/echo "=============================================================================================" >> $RESULT_FAILED_SERVICES
			fi
				return 0	
		fi
	done	
	...
	if [ $flag_user_id -eq 0 -a $flag_group_id -eq 0 ]; then
		...
		/bin/echo "@@$1($2), There is no User= and Group=" >> $RESULT_FAILED_SERVICES
		/bin/echo "==================================================" >> $RESULT_FAILED_SERVICES
	fi	
	if [ $flag_user_id -eq 0 -a $flag_group_id -eq 1 ]; then
		...
		/bin/echo "$1($2), There is no User="  >> $RESULT_FAILED_SERVICES
		/bin/echo "============================" >> $RESULT_FAILED_SERVICES
	fi
	if [ $flag_user_id -eq 1 -a $flag_group_id -eq 0 ]; then
		...
		/bin/echo "$1($2), There is no Group=" >> $RESULT_FAILED_SERVICES
		/bin/echo "============================" >> $RESULT_FAILED_SERVICES
	fi
	if [ $flag_user_id -eq 1 -a $flag_group_id -eq 1 ]; then
		if [ $comment_check_user_id -eq 1 -o $comment_check_group_id -eq 1 ]; then
			...
			/bin/echo "$1($2), Comment like '#User=' and '#Group=' is not allowed" >> $RESULT_FAILED_SERVICES	
			/bin/echo "============================================================" >> $RESULT_FAILED_SERVICES
		else
			if [ $real_uid = $3 -a $real_gid = $4 ]; then
				...
				/bin/echo "$1($2) / expected uid is $3, but real uid is $real_uid" >> $RESULT_FAILED_SERVICES
				/bin/echo "$1($2) / expected gid is $4, but real gid is $real_gid" >> $RESULT_FAILED_SERVICES
				/bin/echo "===============================================" >> $RESULT_FAILED_SERVICES
...
checkWhetherNewRoot(){
	...
	if [ $flag_user_id -eq 0 -a $flag_group_id -eq 0 ]; then
		...
		/bin/echo "$1($2) is new root" >> $RESULT_NEW_ROOT_SERVICES  
		/bin/echo "========================================" >> $RESULT_NEW_ROOT_SERVICES
	fi	
	if [ $flag_user_id -eq 0 -a $flag_group_id -eq 1 ]; then
		/bin/echo "Service path of unknown: $service_path"
		/bin/echo "@@Failed, There is no User="
